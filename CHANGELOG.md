...
- **官网同步前置检查的「回执自噬」死循环**：`_append_website_sync_attempt` 每次失败都更新 `_website-sync-receipt.json`，而该文件就在被扫描的文章目录内 —— 于是「提交回执 → 重跑 finalize → 回执又被更新 → 又判未提交」无限循环（第 90 篇实测卡了三轮，只能人工绕开）。现在把回执与 `_finalize-state.json` 这两个**本次运行自己写的产物**排除出检查集；真正的归档产物（作品库、articles.md、看板、推荐卡）照拦不误，由测试与变异验证钉住。
