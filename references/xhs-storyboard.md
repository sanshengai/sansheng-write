...
### Step 5：无缝跨 Skill 衔接（Agent 自动执行）

为了防止用户在“撰写剧本”和“生成图片”之间出现工作流断档，当本系统输出 `xhs-outline.md` 之后，AI（如 Antigravity / Claude Code）必须**主动自动执行生图工作流**，不要停下来等用户下达新指令：

1. **角色切换**：结束本 skill 的写作环节，在后台隐式唤起 `/baoyu-xhs-images` skill 工作流（`baoyu-xhs-images` 是小红书+微信图文通用接口）。
2. **跳过分析**：因为我们已经交付了顶级精度的 `xhs-outline.md`，直接跳过 `baoyu-xhs-images` 的 Step 1（分析）和 Step 2（跟用户确认方案）。
3. **编译 Prompt**：直接进入它的 Step 3，AI 在后台结合该 skill 内置的 `elements/canvas.md` 和 `presets/warm.md`，把剧本编译成真正的生图提示词并存入其提示词工作目录。
4. **⚠️ 禁止使用参考图链条**：由于小红书干货图文的每一页（网格、流程图、左右对比）排版结构差异巨大，在组装 `batch.json` 或执行生图时，**绝对不要串接 `--ref 01-cover.png`**！这会导致后面所有图的排版被强行锁死成封面的构图，产生极其混乱的乱码贴图。所有的连贯性纯靠 `Style` 定义（如相同的调色板和画风）来维持就足够了。
5. **底层生成**：最后触发底层生图脚本批量产出独立构图的高质量卡片，并在完成后打上水印，一气呵成。
