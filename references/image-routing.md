...
## 封面 ghost 英文只来自 `lead.ghost`

`cover_keywords` 只用于语义检索和视觉概念选择，**不再**被抽词编译成英文水印
（旧机制取不到大写词就让模型自拟，实测编出过重复词）。标题后方的半隐英文
由 `article-meta.yaml:lead.ghost` 显式声明并经编译器校验（2--3 个大写词组、≤ L1、
纯白 8%--14% 不透明），规格与验收见 `cover-styles.md`「英文 ghost 层」。
所有可见文字都必须进入明确白名单，并由视觉质检逐项核对。
