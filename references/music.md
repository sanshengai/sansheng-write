...
2026-09-22 的「图标逻辑」解决了一排缩略图分不开的问题：深底、单主体、无字。但一篇双模型发布快讯
出来的是一张剪开的价签、一块指向两边的路标，作者的判断是
...
2. **名字字面义**：名字自带意象、且**中文读者一眼能懂**时才用——中文名本身（「曾国藩」的「曾」字、
   瑞幸的鹿、京东的狗）或人人认识的英文词。🔴 实测：单画一轮太阳想代表 Sol，46 px 盲配被认成
...
例如两块分开的底座分别托着 5.5 和 6。并置时要写明「中间留出明显空隙」，否则模型会把两个标号挤成一个数（曾有一版出成了「5.56」）。
