...
## 主题曲与播客封面：按图标逻辑出图（2026-09-22 起）

两张封面在读者那里只有一小块：官网迷你播放器 46 px、歌单条目 62 px、播客条目 66 px（CSS px），微信音频卡的封面也差不多。缩到这个尺寸，「场景 + 歌名 + 副标题」的编辑式封面只剩一团暗色，之前 101–105 篇更是清一色书桌台灯落日窗，一排看过去分不出哪首是哪首。所以封面不按海报设计，按 **app 图标** 设计：

- **不放任何文字**。歌名、标题、slogan、logo 一律不进画面；主体自己说明主题。
- **只有一个主体**，居中占画面约 2/3，整个剪影在框内不裁切；没有房间、窗、桌面、风景、天空、地面场景，也没有第二件道具。
- **主体从本篇长出来**：主题曲封面取歌名或某句歌词里最具体的意象（一个物件、一个动作、一个形状），播客封面取文章的核心对象或核心问题的形状，两张主体必须不同——不是同一物件换角度。
- **深底 + 一个强调色**：每张从固定调色板里选一组（`ink / ember / moss / plum / ocean / rust / slate / sand`，全部深底），两张不同；主题曲封面还要避开前三篇用过的。一排缩略图先靠颜色分开，再靠剪影认出来。
- **大块面、高对比**：主体是一个亮的简单形状压在暗底上，没有细纹理、细线、小零件；判据是 64 px 还认得出。渲染质感可选 `sculpted`（默认，哑光雕塑体）/ `papercut` / `flat` / `painted`，两张可以不同。

**怎么想主体**：先问「这首歌 / 这篇文章如果只能画一样东西，画什么」，再问「那样东西在 64 px 下靠轮廓认得出吗」。认不出的换更简单的形（天平比表格好、一级亮起的台阶比一张榜单好、一只信封落进托盘比一个流程图好）。窗、台灯、书本、河流、小船、落日、耳机、麦克风只有歌词或正文依据里真出现了才允许，脚本会拦。

出图前在文章目录写 `_audio-cover-plan.json`（`schema_version: 2`）。`article_anchor` 至少 8 字符、必须是 `定稿.md` 原文；`lyric_anchor` 必须是生成单里真实的歌名或歌词句；`subject` 只写一个主体，不列清单、不写场景、不含文字类词。示意结构（字段必须换成当前文章的实际内容）：

```json
{
  "schema_version": 2,
  "theme_cover": {
    "subject": "四级台阶只有最低一级亮着，一个小小的无脸人影站在那一级上",
    "lyric_anchor": "凌晨那一档亮了",
    "article_anchor": "正文中的具体原文",
    "palette": "ocean",
    "render": "sculpted"
  },
  "podcast_cover": {
    "subject": "一把老式天平，两端各放一枚大小接近的砝码，稍微向一侧倾斜",
    "article_anchor": "正文中的具体原文",
    "palette": "ember",
    "render": "flat"
  }
...
`audio_covers.py` 会核对上述全部规则，缺失或违规直接报错，不用通用景物占位；原文命中只是取材检查，不能证明图片贴题。出图后脚本写 `_audio-cover-thumbs.png`，把两张按 256 / 128 / 64 / 46 px 排成一行——**看这张验收，不看 1024 原图**：46 px 下主体轮廓还在、两张一眼能分开、与前三篇的 `_audio-cover-thumbs.png` 并排不像同一套，才算过；画面里冒出任何文字、主体被切、出现第二件道具或背景场景，改 `subject` 后 `--force` 重生成。核对结论写 `_audio-cover-review.md`。修改已有封面走 `--force` 或 handoff 的 `--regenerate-covers`；不自动改动历史已发布图片。
