...
**重渲多了就跑一次 `render-stats`**（不改任何状态，纯读 `素材/.render-attempts.jsonl`）：

```bash
python "$SKILL/scripts/pipeline.py" render-stats
```

它按图打印「渲了几次 / 成功几次 / QA 打回几次 / 产出过几张不同的图」，
并算出浪费量。🔴 **不要凭印象判断某张图是不是渲多了** —— 第 89 篇那次
「45 次渲染、6 张图、浪费 39 次」是事后靠手工数出来的，这个命令就是来替掉那个活的。

排查顺序**按实测收益从高到低**（2026-08-16 的 60 张对照实验，详见
`_ops/生图首过率改造-实验与结论-20260816.md`）：

1. **frontmatter 有没有泄漏给模型**（67% → 92%）—— 现已由 `.render-body/` 自动剥离
2. **SCENE 有没有具体物象**（50% → 92%）—— 只写 block/node/cluster 时模型会拿文字填画布
3. **文字之间是否共享 ≥2 个连续字**（17% → 50%）
4. **layout 有没有自带文字的物件**（日历/报纸/招牌/屏幕/图表）

第 2-4 条都有编译期闸门，`compile-visuals` 就会拦；真渲到一半才发现，
先怀疑是不是绕过了编译期（比如手工改了 prompt 文件）。
