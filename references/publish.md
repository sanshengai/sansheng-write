...
2. 作品库归档及派生视图刷新。
3. archive 验证。
4. **生成 `_moments-copy.md`**；实际朋友圈发布仍由作者完成。🔴 自 2026-09-23 起，文案在 `release-to-draft` 成功时就已生成、改写并交付给作者（见 release-runtime.md §5 第 7 步）；这里通常是保留终稿的空跑，只有草稿阶段漏交付时才在这里补。
...
`_moments-copy.md`（新文章在 `过程记录/` 里，命令输出会打印实际路径）是可整段直接粘贴的纯文本，必须满足以下协议：
