...
式封面缩到那个尺寸只剩一团暗色；101–105 篇更是清一色书桌台灯落日窗，分不出
哪首是哪首。现在按 **图标逻辑** 出图：
...
def _check_subject(stage: str, item: dict, evidence: str) -> None:
    ...
    for word in TEXT_WORDS:
        if word in subject:
            raise ValueError(f"{stage}.subject 含「{word}」；封面不放任何文字，主体本身要能说明主题")
...
def _cover_plan(article_dir: Path, stages: list[str], body: str) -> dict[str, dict]:
    ...
    if not isinstance(plan, dict) or plan.get("schema_version") != PLAN_SCHEMA:
        raise ValueError(
            f"{COVER_PLAN} 的 schema_version 必须为 {PLAN_SCHEMA}（图标逻辑：无文字、单主体、调色板）；"
            "旧的 scene/display_title 规划按 music.md 重写"
        )
    ...
    for stage in stages:
        ...
        if stage == "theme_cover":
            ...
            evidence += lyric
        _check_subject(stage, item, evidence)
        ...
        if "display_title" in item or "scene" in item:
            raise ValueError(f"{stage} 仍带 scene / display_title；schema {PLAN_SCHEMA} 只认 subject，封面不放字")
...
def _icon_rules(plan: dict) -> str:
    ...
    return (
        f"ONE subject only: {plan['subject']}\n"
