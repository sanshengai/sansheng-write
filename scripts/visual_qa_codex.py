...
CHECK_DEFINITIONS = {
    ...
    "ghost_layer_subdued": (
        "封面标题后方必须有一行半隐的全大写英文（ghost 层），且它必须是**背景纹理**："
        "判 true 需同时满足：① 能看见这行英文，位置在中文主标题后方或被主标题局部压住；"
        "② 它明显比主标题安静 —— 近背景色的低对比（约 8%-14% 不透明度），字高不超过主标题；"
        "③ 缩略图尺寸下第一眼仍落在中文主标题上。"
        "完全没有这行英文、或它比主标题更亮更大更抢眼、或它是随机字母/乱码而非可读词组，判 false 并写明是哪一种。"
        "这行英文的具体内容已在白名单里；因不透明度太低读不全不算违规，但读出来的字母必须与白名单一致。"
    ),
