...
def render_top_principles(meta: dict) -> str:
    ...
    if not opening:
        ...
        if "资讯直入" in _style:
            opening = "直入"
    if opening:
        # 深度启发类同样结论先行（writing.md §三条直球守则）：旧值「可钩子」保留为别名，
        # 但语义已收紧 -- 场景只做结论的支撑，不再允许悬念铺陈与延迟揭示。
        _deep_hint = (
            "🔴 **深度启发类结论先行**：核心判断落在第一屏（约前 300 字）；"
            "可用真实场景 / 案例开篇，但它是已说出口结论的支撑，不是卖关子 -- "
            "不设悬念、不埋倒钩、不延迟揭示。现实写作只用真实亲历或可追溯公开案例，"
            "虚构 / 混合写作必须显式标明。"
        )
        _opening_hint = {
