...
- **基线数据**（2026-09-23 量的，`<数据目录>/*/素材/cover.png`，
  含最近 9 篇的全部现存封面，外加更早的扩大样本，共 99 张只读实测，
  不进本仓）：
  - ghost 层：99 张里 81 张（82%）测到候选像素簇；测到的里只有 27 张（33%）
    落在契约写的 8%-14% 范围内，其余明显偏淡或偏浓（样本中有低至约 5.7%、
    高至约 16.0% 的）；最近 9 篇这个必测区间里 9 张全部测到候选像素簇，
    但只有 2 张落在 8%-14%，其余 7 张在 4.8%-7.5% 之间
    （包括肉眼确认过、观感正常但实测 7.5% 的样本）。
  - 结论：契约数字是**设计目标**，不是当前生产分布的真实约束；拿
    `in_contract_range` 当硬门会把大量已发布、作者认可的封面判 fail，
    所以只接成 warn。
  - 水印：99 张里 96 张（97%）测到符合 35% 不透明度特征的像素簇；
    最近 9 篇必测区间 9 张全部命中（100%）。另外 3 张未命中的
    早期封面经目视核实是**引入 `add_logo.js` 水印惯例之前的
...
# 永远不会触发。99 张真实基线里最高的是 0.486，故取 0.49，
...
# 命中率只有 54%（其中肉眼可见水印的样例，仍判「未检测到」，因为水印正左侧
