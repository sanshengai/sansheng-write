...
| 播客（RSS / 公众号 / 官网） | 把定稿做成双主持音频并复用到 RSS、官网；显式设 `wechat_embed: true` 后，公众号导读后也会出现与主题曲同级的播客卡 | NotebookLM 登录态、ffmpeg、一台放 mp3 与 `feed.xml` 的主机（可 SSH） | 同上；未显式开启嵌入时公众号保持主题曲单卡 |
